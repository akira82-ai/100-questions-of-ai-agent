...
Too many plugins make the vault slow, which makes it easier to quit. This chapter is not about filling your vault with plugins. It is about which ones to install, which ones to skip, and a few plays worth knowing exist. For the advanced stuff, knowing the road is there is enough; do not rush down it on day one. However flashy a plugin is, it serves what you accumulate. The root is still your vault.

## 83. Does Obsidian have official AI? If not, how to pick community plugins?

Someone installed forty plugins in six months and added not a single note. Obsidian does not ship built-in AI, and every AI capability comes from community plugins. Waiting for an official feature is waiting forever, so judging plugins is a skill you have to build yourself.

Use three hard checks before installing anything. Put it back unless you get three yeses. First, update frequency: open the GitHub repo and confirm the latest commit is within six months, which tells you the author is still around. Second, issue response: sort issues by newest and read the last ten; if the author replied to more than half, you have a maintainer to ask when things break. Third, scope: the top of the README should explain the plugin in one sentence. A plugin bundling seven unrelated features brings a maintenance risk and a performance cost together.
...
Keep your always-on plugin count under three. For every extra one, you should be able to name the step it removes from your day. On the personal vault plus AI track, only three capabilities earn a permanent slot: asking questions against your notes, surfacing old notes while you write, and running a model locally.

The concrete mapping: Copilot handles note-based Q&A and acting on selected text. Smart Connections pushes related old notes into the sidebar as you type. Ollama pulls the model onto your machine, and once Copilot points at that local address, your chats stop leaving the laptop. Install all three, use them for two weeks, and disable whichever one you never opened on purpose.
...
Depends on: Copilot plugin + Ollama
...
Do not build a system from a blank page, and do not download three starter vaults to compare. Pick the one closest to what you actually do this month, then replace its sample content with three real notes of your own the same day. If your material will not fit, that structure is wrong for you. Delete it and try the next one.

A few reliable entries. Vaultorial collects free MIT-licensed starter vaults sorted by daily notes, tasks, students, and Zettelkasten, downloaded as a zip with no sign-up. The Vault Templates section of Obsidian Garden Gallery leans systematic, with full PARA and Zettelkasten frameworks, and you can browse other people's digital gardens while you are there. For an MOC-style opening with Chinese-language docs, LYT Kit has a Mandarin repo you can clone and edit.

Once you have it, clear the stage first. Move every sample note outside the template folder into a holding directory and keep only the folder skeleton and the template files. Leave the samples in and you will keep reading another person's vault instead of writing your own.
...
Resource: Vaultorial https://vaultorial.com · Obsidian Garden Gallery https://vaults.obsidian-community.com/ · LYT Kit in Mandarin https://github.com/nickmilo/LYT-Kit-in-Mandarin
...
The action you can take today is to measure your own link quality. First list notes with more than 20 outgoing links; most of them are junction pages polluted by autocomplete, so cut each down to five links you can explain out loud. Then list long notes with zero outgoing links and give each one real parent link. After those two passes, graph retrieval gets visibly sharper.
...
Depends on: Local REST API plugin (for the MCP client to connect)
...
Depends on: Local REST API plugin
...
Someone subscribed to a dozen sources, copied every new trick the day they saw it, and two months later had a vault full of half-finished configs. Keep your source list under five, favor weekly digests, skip anything that publishes daily. The tutorial sea drowns people because it updates far faster than you practice, so you stay busy chasing while your vault stays empty.

Three types are worth keeping. A community weekly that rounds up new plugins and known pitfalls, ten minutes a week. A live Q&A floor, where searching your exact error message on r/ObsidianMD beats watching a tutorial. And one index list: awesome-obsidian sorts plugins, themes, and templates by category, so check it before you go hunting.
...
Resource: Obsidian Roundup https://obsidianroundup.org · r/ObsidianMD https://www.reddit.com/r/ObsidianMD/ · awesome-obsidian https://github.com/kmaasrud/awesome-obsidian
...
Someone opened the plugin browser, watched it spin, and got a load failure. Set one rule first: third-party directories are for research only, where you compare downloads, update activity, and betas that have not reached the official market yet. The actual download goes through the official market or the author's GitHub Releases page. A plugin is code that can read every file in your vault, and every extra hop in its supply chain is extra risk.

When the market will not load, work through it in order. Check whether the network is the problem: if the browser opens GitHub while the in-app market does not, the client's channel is blocked. Next, install by hand, pulling three files from the author's Releases page into your plugins folder, which depends on no acceleration service at all. If you do need a mirror service, pick one whose version numbers match the official repo.

Three red lines when picking: no update in six months, network permission with no explanation of what gets sent, and an archived repo. Any one of them, walk away.
...
Resource: PKMer https://pkmer.cn · Obsidian Stats https://www.obsidianstats.com · Official plugin directory https://obsidian.md/plugins

## 91. My vault gets slower with use, how to health-check and slim down?

Someone waits fifteen seconds for the vault to open and reaches straight for the delete key, which is backwards. Slowness comes from three places: the number of always-on plugins, the size of your attachments, and the file count in the vault. Work through them in that order, and for most vaults the first two solve it.

Step one, count plugins. Check the enabled count in settings, and above twelve start disabling in batches, restarting after each batch and writing down the startup time. Step two, audit attachments. Run a command at the vault root to find files over 5 MB, then move big PDFs and screen recordings outside the vault and reference them by absolute path. Step three is the content check, with the three Dataview queries below.

Here is what healthy looks like. Orphan notes under 15% of the vault is fine. Notes missing frontmatter should approach zero, because they are invisible to every query you will ever write. More than 30 notes under 200 bytes means the inbox is backing up, so clean the inbox. Handle whatever the queries return the same day rather than saving it for a later round.
...
Depends on: Dataview plugin; use Find orphaned files and broken links for the broken-link list

Resource: Dataview https://community.obsidian.md/plugins/dataview · Find orphaned files and broken links https://community.obsidian.md/plugins/find-unlinked-files

## 92. Want to migrate away from Obsidian, can local Markdown really leave anytime?

Someone running a vault of forty thousand files migrated by doing exactly one thing: copying the folder to a new machine. You can leave anytime, as long as your body text has not been captured by plugin syntax. The self-test is direct: disable every community plugin, restart, and open ten notes at random. If the screen fills with code blocks and fields that no longer render, that content is now tied to a plugin.

Count before you move. Dataview blocks, tasks markers, and excalidraw files all degrade into raw text in another editor. If there are only a few, rewrite them as static content by hand. If there are many, gather them into one folder and deal with them separately at migration time so they stay out of your body text.
...
Resource: Pandoc https://pandoc.org · Obsidian Help https://help.obsidian.md
